...
class TodosController < ApplicationController
  ...
  def olddb_todo_create(params)
    @old_todo = OldTodo.create(issued_department: ((params[:department_id] != '') && !params[:department_id].nil? ? Department.find(params[:department_id]).sync_id : nil), status: 1, title: params[:title], time: params[:eta], description: params[:description])
    @old_todo.id
  end
